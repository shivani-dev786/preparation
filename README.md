...
- Solved **LeetCode #424 - Longest Repeating Character Replacement** ✅
- Learned the **Sliding Window + Frequency Count** approach.
- Used a dictionary `count` to track character frequencies.
- Used `left` and `right` pointers to maintain the sliding window.
- Tracked the most frequent character using `maxfreq`.
- Learned the key formula:

  `window_size - maxfreq <= k`

- If replacements required were greater than `k`, the window was invalid:
  - Decreased the frequency of `s[left]`.
  - Moved `left` forward.
- Updated the maximum valid window length using `maxlength`.
- Successfully submitted the solution and got **Accepted** 🎉
...
### Day 62 🔄 — Minimum Window Substring (#76)
- Continued LeetCode #76 - Minimum Window Substring
- Sliding Window + HashMap / Frequency Count
- `count` stores required character frequencies from `t`
- `window` stores character frequencies in the current window
- `need` = number of unique characters required
- `have` = number of required characters whose frequency is satisfied
- When `have == need`, the current window is valid
- Track the smallest valid window using `minlen` and `start`
- Shrink the window from the left to find a smaller valid substring
...
### Day 72 — Capacity To Ship Packages Within D Days

**LeetCode #1011 — Capacity To Ship Packages Within D Days**  
**Pattern:** Binary Search on Answer

#### Problem
Given an array of package weights and a number of days, find the **minimum ship capacity** required to ship all packages within the given number of days.
